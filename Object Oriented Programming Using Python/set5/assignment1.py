...
class PermanentEmployee(Employee):
    ...
    def identify_performance_hike(self):
        list=self.get_performance_list()
        if list[4]==1 and list[3]==1:
            hike=5
        elif list[4]==1 and list[3]==2 and list[2]==1:
            hike=3
        else:
            hike=None
        return hike
    ...
    def identify_job_level_hike(self):
        if self.get_job_level() in Company.dict_hike:
            job_hike=Company.dict_hike[self.get_job_level()]
        else:
            job_hike=None
        return job_hike
    
    def identify_incentive(self):
        total_incentives=self.__p_incentive+self.get_e_incentive()+Company.get_c_incentive()
