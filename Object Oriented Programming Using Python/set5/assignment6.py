...
class PassengerLogistics(Logistics):
    ...
    def calculate_bill_amount(self):
        if self.validate_vehicle_type()!=-1 and self.validate_meter_reading():
            ...
            ind=PassengerLogistics.__list_vehicle.index(self.__vehicle_type)
            if total_distance<=100:
                bill_amount=total_distance*PassengerLogistics.__list_charge_for_hundred[ind]
            else:
                bill_amount=(total_distance-100)*PassengerLogistics.__list_charge_after_hundred[ind]+(100*PassengerLogistics.__list_charge_for_hundred[ind])
                
            if bill_amount<PassengerLogistics.__list_minimum_charge[ind]:
                bill_amount=PassengerLogistics.__list_minimum_charge[ind]
    ...
    def calculate_bill_amount(self):
        if(self.validate_carrier_type()):
            if(self.validate_meter_reading()):
                power=self.__goods_weight/1000-1
                self.generate_consumer_id()
                total_distance=self.get_end_reading()-self.get_start_reading()
                if(self.__goods_weight<=1000):
                    ...
                    charge_per_kilometer=GoodsLogistics.__carrier_dict[self.__carrier_type]*2**power
                else:
