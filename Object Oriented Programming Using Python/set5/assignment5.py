...
class Graduate(Employee):
    ...
    def calculate_gross_salary(self):
        ...
        if self.validate_job_band() and self.validate_qualification() and self.validate_basic_salary():
            salary=self.get_basic_salary()+0.12*self.get_basic_salary()+((i[self.get_job_band()]/100)*self.get_basic_salary())
...
class Lateral(Employee):
    def __init__(self,job_band,skill_set,employee_name,basic_salary,qualification):
        super().__init__(job_band,employee_name,basic_salary,qualification)
    ...
    def calculate_gross_salary(self):
        l={"AGP":6500,"AGPT":8200,"AGDEV":11500}
        i={"D":13,"E":16,"F":20}
        if self.validate_job_band() and self.validate_qualification() and self.validate_basic_salary():
            salary=self.get_basic_salary()+0.12*self.get_basic_salary()+((i[self.get_job_band()]/100)*self.get_basic_salary())+l[self.__skill_set]
            return salary
        return -1
    def validate_job_band(self):
