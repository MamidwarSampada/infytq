...
def create_new_sentence(word_list):
    ...
    while temp is not None:  
        if temp.get_data().isalpha(): 
            new_sentence+=temp.get_data()  
        elif temp.get_data()==temp.get_next().get_data()=="/": 
           # new_sentence+=""
            temp.get_next().get_next().set_data(temp.get_next().get_next().get_data().title())
        elif temp.get_data()==temp.get_next().get_data()=="*": 
            #new_sentence+="" 
            temp.get_next().get_next().set_data(temp.get_next().get_next().get_data().title()) 
...
l=[]
    ...
    for i in range(len(l)):
        if l[i] not in ['*','/'] :
            new_sentence+=l[i]
        elif( i<len(l)-2 and l[i] in ['*','/'] and l[i+1] in  ['*','/']):
            l[i+2]=l[i+2].upper()
        else:
            new_sentence+=" "
    # Write your logic here
